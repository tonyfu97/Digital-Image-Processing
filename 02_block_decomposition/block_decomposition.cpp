...
int main(int argc, char **argv)
{
    ...
    CImg<unsigned char> mask = 1 - (res.get_shift(1, 1, 0, 0, 0) - res).norm().cut(0, 1);
    
    mask.get_normalize(0, 255).save(("results/" + file_basename + "_mask.png").c_str());
    res.save(("results/" + file_basename + "_res.png").c_str());
    res.mul(mask);
